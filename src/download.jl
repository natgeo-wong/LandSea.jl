...
function getLandSea(
    ...
    if savelsd
        ...
        if !isfile(lsmfnc)
            ...
            if !isfile(glbfnc); setup(type,path,resolution) end
        ...
        if returnlsd
            ...
            return LandSeaData{FT}(lon,lat,lsm,oro,msk)
        ...
        return LandSeaData{FT}(ggrd.lon,ggrd.lat,rlsm,roro,mask)
...
function download(
	...
	save(GeoRegion("GLB"),lon,lat,lsm,oro,mask,path,type,resolution)
